        ...
        if (episode_steps >= max_step and max_step):
            if step > args.warmup:
                ...
                if episode > 0 and validate_interval > 0 and episode % validate_interval == 0:
                    ...
                    print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), '{:10d} Step_{:07d}: mean_reward:{:.3f} mean_dist:{:.3f} var_dist:{:.3f}'.format(int(time.time() - starttime), step - 1, np.mean(reward), np.mean(dist), np.var(dist)))
...
if __name__ == "__main__":
    ...
    # hyper-parameter
    parser.add_argument('--warmup', default=500, type=int, help='timestep without training but only filling the replay memory')
    ...
    parser.add_argument('--validate_episodes', default=50, type=int, help='how many episode to perform during validation')
